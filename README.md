...
- ユーザーの言語に合わせてメッセージを自動翻訳することができます。
...
まずメインスクリプト（app.jsやindex.js）でbot-expressをインポートします。
...
app.use('/webhook', bot_express({
    ...
    auto_translation: 'enable' または 'disable', // オプション。自動翻訳を有効にする際に'enable'に設定
}));
```
...
メインスクリプトのbot-expressの初期設定においてgoogle_project_idとauto_translationを設定することによって自動翻訳を有効化できます。
これはbot-express側での記述言語に依らず、発話したユーザーの言語に応じて会話を自動的に翻訳する機能です。

例えば、bot-expressのskillファイルではメッセージをすべて日本語で記載しているとします。自動翻訳を有効化した状態でとあるユーザーが英語でBotに話しかけて来ると、そのユーザーからのメッセージは日本語に翻訳されてbot-expressで処理されます。そしてユーザーに返信がある場合、メッセージは日本語から英語に翻訳されて送信されます。
...
LINE
- text messageのテキスト、template messageのタイトル、テキスト、代替テキスト、actionのテキストが翻訳されます。
- template messageのpostbackボタンのデータは翻訳されません。

Facebook Messenger
- text messageのテキスト、quick replyのテキスト、template messageのタイトル、サブタイトル、ボタンのタイトルが翻訳されます。
- template messageのpostbackボタンのペイドードは翻訳されません。
