...
Node.jsで稼働するChat Botを開発するためのフレームワーク。コンシェルジュ型のBotに必要とされる自然言語処理、文脈の理解、必要なパラメータの収集、LINEやFacebookといった対応しているメッセージプラットフォームへのAPIコール機能などが搭載されており、開発者はフォーマットに従って「skillファイル」を追加するだけでBotの能力を拡張できることができます。

bot-expressは複数のメッセージプラットフォームに対応しており、開発者が追加したスキルはサポートされるすべてのメッセージプラットフォームで動作します。現在対応しているメッセージプラットフォームはLINEとFacebookです。

# アーキテクチャー

# bot-expressを使うことで何が嬉しいのか？

例えばスマートホームのコンシェルジュとなるBotを開発するとします。家にはPhilips社のHue（IoT電球。On/Offや照明色の変更が可能）が設置されており、プログラマティックに照明の色が変更できるようになっています。Botにこの照明の色を変更させるには、例えばユーザーは下図のようにBotにリクエストすると予想されます。

「ライトの色を変えて」

この時Botに必要とされる処理は、 **このメッセージからユーザーが照明の色の変更をリクエストしていると認識すること(意図の特定)** と、 **何色にすればいいのか確認すること（パラメーターの収集）** 、そして最後に **照明の色を変更すること（最終処理）** です。
...
人間だと柔軟に対応できるこういった臨機応変さをプログラムに組み込むのはいささか骨の折れる作業ですが、bot-expressはこのパラメータ収集作業をかなり軽減してくれます。開発者はBotのスキル（前述の例だと照明の色を変える能力）を追加する際、そのスキルにおいて最低限必要なパラメーターをJSONフォーマットで設定します。
